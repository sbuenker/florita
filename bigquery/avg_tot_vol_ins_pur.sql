# the steps below subset the original FEMA policies dataset based on variables needed to determine voluntary purchases of flood insurance
# additionally, the steps also calculate the following by occupancyType/buildingType/sfha/county/state/year observation:
...
# step 6. encode missing values for totalBuildingInsuranceCoverage and totalContentsInsuranceCoverage as zero
cte6 AS (
  SELECT
    CASE
...
# step 7. calculate all the averages and totals mentioned above
SELECT
  countyCode,
  propertyState,
  year,
...
FROM cte6
WHERE countyCode IS NOT NULL
GROUP BY countyCode, propertyState, year, buildingType, occupancyType, sfha_flood_risk
ORDER BY countyCode, propertyState, year, buildingType, occupancyType, sfha_flood_risk
