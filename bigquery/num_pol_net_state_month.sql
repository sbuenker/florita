# the steps below:
    ...
    # calculate the purchases, terminations and net policies bought in any given month in any given state
...
# output: generates SQL table which is subsequently saved as "num_pol_net_state_month.csv" 
...
# step 6. determine which policies are bought in any given month
pol_eff AS (
  SELECT
    year AS year_eff,
    EXTRACT(month FROM policyEffectiveDate) AS month_eff,
    propertyState,
...
pol_term AS (
  SELECT
    ...
    EXTRACT(month FROM policyTerminationDate) AS month_term,
    propertyState,
...
# calculate net bought policies in any given month
pol AS (
  SELECT
    pol_eff.year_eff AS year,
    pol_eff.month_eff AS month,
  ...
  ON pol_eff.year_eff = pol_term.year_term AND 
     pol_eff.month_eff = pol_term.month_term AND
